/* eslint-disable prettier/prettier */
let id = 3000;

// eslint-disable-next-line no-plusplus
const sumId = () => id++;

const repertorioMissaFev = [
  {
    titulo: 'Mantos e Palmas',
    id: sumId(),
    categoria: 'Entrada',
    link: 'https://youtu.be/cWN7pijjiQM',
    base: false,
    cifra: `
    Intro: A Bm E A
    A    Bm
    :ho-sana,
     E           A
    :ho-sana ao rei
    A      Bm
    :ho-sana,
     E           A
    :ho-sana ao rei

     A                          Bm
    Mantos e palmas espalhando vai
    E                    A
    O povo alegre de jerusalém
                                Bm
    Lá bem ao longe se começa a ver
     E                           A   G#m  F#m
    O filho de Deus que montado vem.

    F#m                           C#m
    Enquanto mil vozes ressoam por ai
    D                             A     G#m F#m
    Hosana ao que vem em nome do senhor
                               C#m
    Com um alento de grande exclamação
    D                       E
    Prorrompem com voz triunfal!

    A                     Bm
    Como na estrada de jerusalém
    E                        A
    Um dia também poderemos cantar
                                  Bm
    A Jesus cristo que virá outra vez
    E                        A  G#m    F#m
    Para levar-nos ao eterno lar

     F#m                             C#m
    Enquanto mil vozes ressoam por ai
    D                            A     G#m F#m
    Hosana ao que vem em nome do senhor
                              C#m
    Com um alento de grande exclamação
    D                       E
    Prorrompem com voz triunfal!

    [refrão]

    `,
  },
  {
    titulo: 'Perdoa-me',
    id: sumId(),
    categoria: 'Penitencial',
    link: 'https://youtu.be/NUUfEDkeXrQ',
    base: false,
    cifra: `
    [Intro] E  B/D#  E  F#m7  E
        B/D#  E  B/D#  C#m
        B/D#  F#m7  E9

    [Primeira Parte]

                      B/D#    E9
    Senhor, que Te deixaste ferir
            E/G#   A  A/C#    E9
    Do Teu sangue vem      a paz!
            F#m7    C#m7  B4  B
    Aqui estou, perdoa___me!

    [Refrão 1]

    C#m7   B/D#  E4  E
    Ky____rie   elei_son
    E  B/D#  C#m  B   A
    Ky_ri_____e      eleison
    B/D#  E  F#m7  E/G#    A9
    Ky____ri__e           eleison
      C#m7  B  E9
    Ôoooo

    [Segunda Parte]

        E9        B/D#  E9
    Oh Cristo, elevado  na cruz
        E/G#  A9  A/C#  E9
    És amigo  do  pe____cador!
            F#m7    C#m7  B4  B
    Aqui estou, perdoa___me!

    [Refrão 2]

      C#m7  B/D#   E4  E
    Chris___te    elei_son
      E  B/D#  C#m  B   A
    Chris______te      eleison
      B/D#  E  F#m7  E/G#   A9
    Chris______te          eleison
    C#m7  B  E9
    Ôoooo

    [Terceira Parte]

        E9       B/D#      E9
    Senhor,  da morte, vencedor
          E/G#  A9  A/C#  E9
    Verdadeiro fi_lho    de Deus!
            F#m7    C#m7  B4  B
    Aqui estou, perdoa___me!

    [Refrão 1]

    C#m7   B/D#  E4  E
    Ky____rie   elei_son
    E  B/D#  C#m  B   A9
    Ky_ri_____e      eleison
    B/D#  E  F#m7  E/G#    A9
    Ky____ri__e           eleison

    [Final] C#m7  B  E9
            C#m7  B  E9
    `,
  },
  // {
  //   titulo: 'Glória a Deus Nas Alturas - Ministério Amor e adoração',
  //   id: sumId(),
  //   categoria: 'Gloria',
  //   link: 'https://www.cifraclub.com.br/ministerio-amor-e-adoracao/gloria-a-deus-nas-alturas/',
  //   base: true,
  //   cifra: `
  //   E                  B9
  //   Glória a Deus nas alturas
  //     A9                 B9
  //   Glória a Deus nas alturas
  //      C#m              B9              A9
  //   E paz na terra aos homens por Ele amados (2x)

  //   [Primeira Parte]

  //           F#m          G#m        A
  //   Senhor Deus rei dos céus, Deus pai todo poderoso
  //               C#m7            B9
  //   Nós vos louvamos, vos bendizemos
  //               A
  //   Nós vos adoramos e glorificamos
  //    F#m            G#m        A             B9
  //   Nós vos damos graças, por vossa imensa glória

  //   [Refrão]

  //     E                  B9
  //   Glória a Deus nas alturas
  //     A9                 B9
  //   Glória a Deus nas alturas
  //      C#m              B9              A9
  //   E paz na terra aos homens por Ele amados

  //   [Segunda Parte]

  //       F#m        G#m    A
  //   Senhor Jesus Cristo, filho unigênito
  //           C#m7              B9    A             B9
  //   Senhor Deus, cordeiro de Deus, filho de Deus pai
  //    C#m                B9            A                B9  G#
  //   Vós que tirais o pecado do mundo tende piedade de nós
  //    F#m7              G#m7           A                B9
  //   Vós que tirais o pecado do mundo, acolhei a nossa súplica
  //    C#m7               B9           A                B9   G#
  //   Vós que estais a direita do Pai tende piedade de nós
  //       F#m        G#m       A        B9
  //   Só vós sois o santo, só vós o Senhor
  //       C#m7    B9              A
  //   Só vós o altíssimo, Jesus Cristo
  //          F#m              A             B9   E  B9
  //   Com Espírito Santo na glória de Deus pai amém

  //   [Refrão]
  //   E                  B9
  //   Glória a Deus nas alturas
  //     A9                 B9
  //   Glória a Deus nas alturas
  //      C#m              B9              A9
  //   E paz na terra aos homens por Ele amados (2x)
  //   `,
  // },
  {
    titulo: 'Meu Deus, Meu Deus, Por Que Me Abandonastes',
    id: sumId(),
    link: 'https://youtu.be/qBklqaIubg8',
    base: false,
    categoria: 'Salmo',
    cifra: `
    Em                              Am
    Meu Deus, meu Deus, por que me abandonastes?
    C                     B7                 Em  B7
    Meu Deus, meu Deus, / por que me abandonastes?



    Em                 B7                 Em
    Riem de mim todos aqueles / que me veem,
    Am                                 B7
    Torcem os lábios e sacodem a cabeça.
    C                                B7
    'Ao Senhor se confiou, ele o liberte
    Am                B7                Em        B7
    E agora o salve, se é verdade que ele / o ama!'


    Em               B7         Em
    Cães numerosos me rodeiam furiosos,
    Am                                           B7
    E por um bando de malvados eu fui / cercado.
    C                                               B7
    Transpassaram minhas mãos e os meus pés,
    C                                          B7
    E eu posso contar todos os meus ossos.
    Am           B7                Em        B7
    Eis que me olham e, ao ver-me, se / deleitam!


    Em              B7                   Em
    Eles repartem entre si as minhas vestes,
    Am                               B7
    E sorteiam entre si a minha túnica.
    C                                                   B7
    Vós, porém, ó meu Senhor, não fiqueis longe,
    Am        B7                         Em    B7
    Ó minha força, vinde logo em meu / socorro!



    Em              B7                     Em
    Anunciarei o vosso nome a meus irmãos
    Am                                           B7
    E no meio da assembleia hei de / louvar-vos!
    C                                                  B7
    Vós que temeis ao Senhor Deus, dai-lhe louvores,
    C                                       B7
    Glorificai-o, descendentes de Jacó,
    Am           B7             Em      B7
    E respeitai-o toda a raça de Israel!
    `,
  },
  {
    titulo: 'Louvor a vós, ó Cristo Rei',
    id: sumId(),
    link: 'https://youtu.be/-clhbTEQJ_8',
    base: false,
    categoria: 'Aclamacao',
    cifra: `
    Intro: Dm G Dm G C Dm
    Dm           G      Dm
    Louvor a vós, ó Cristo Rei,
    G      Dm     G      Dm            C  Dm
    Rei da eterna glória, Rei da eterna glória.

      Dm           Am
    Oxalá ouvísseis hoje a tua voz
           Dm            G        Gm      Dm
    Não fecheis os corações. Como no deserto

    `,
  },
  {
    titulo: 'Pão e vinho',
    id: sumId(),
    categoria: 'Oferta',
    link: 'https://www.youtube.com/watch?v=szAXJgaI_kI',
    base: false,
    cifra: `
    Tom: A

    [Primeira Parte]

           A9              E          F#m7
    Pão e vinho te apresentamos nesse altar
          C#m7              D9         C#m7
    Como sinal que tu recolhes nossa oferta
                Bm7               E
    Tudo o que somos deixamos aqui

           A9              E          F#m7
    Pão e vinho te apresentamos nesse altar
          C#m7              D9         C#m7
    Como sinal que tu recolhes nossa oferta
                Bm7              E
    Tudo o que somos deixamos aqui

    [Refrão]

    A/C#    D9           C#m7
    É um milagre que se dá
               D9               F#m7
    O pão e o vinho em corpo e sangue
             Bm7      E
    Vão se transformar
     A/C#     D9          C#m7
    Não ha limites para o amor
                 D9               F#m7
    Vem transformar também minha vida
           Bm7    E                  A9  F#m7 Bm7
    Oh senhor, é teu esse milagre de amor
    [Final]

       E                    A9  F#m7 Bm7
    É teu esse milagre de amor
       E                   A9
    É teu esse milagre de amor
    `,
  },
  {
    titulo: 'Santo É o Senhor Meu Deus',
    id: sumId(),
    categoria: 'Santo',
    link: 'https://www.youtube.com/watch?v=U9jVob3i8T8',
    base: false,
    cifra: `
    Tom: C

    Am            G
    Santo é o Senhor meu Deus
    F           E
    Digno de louvor
       Am     G       F          E
    A Ele o poder, a honra e a glória (BIS - MULHERES)

      Am    G        F           E
    Hosana, hosana, hosana ao Senhor (BIS)

      Am         G     F        E
    Bendito que vem em nome do Senhor
          Am     G
    Com todos os Santos
       F       E
    Cantamos pra Ti (BIS - MULHERES)

    Refrão
    `,
  },
  {
    titulo: 'Cordeiro de Deus',
    id: sumId(),
    categoria: 'Cordeiro',
    link: 'https://www.youtube.com/watch?v=dOOJTDGAwIw',
    base: false,
    cifra: `
    [Intro]  G Bm C D

        G         Bm7               C9
    Cordeiro de Deus que tirais o pecado do mundo
           Am7       D           G    D/F#
    Tende piedade, piedade de nós
        G         Bm7               C9
    Cordeiro de Deus que tirais o pecado do mundo
          Am7       D           G     D/F#
    Tende piedade, piedade de nós
        G         Bm7               C9
    Cordeiro de Deus que tirais o pecado do mundo
           Am7     D/G  D7  G
    Dai-nos a paz, a vossa paz
    `,
  },
  // {
  //   titulo: 'Amém',
  //   id: sumId(),
  //   categoria: 'Novas',
  //   link: 'https://www.youtube.com/watch?v=ZpafvInYt1w&list=PL9F8jR3RBb9I30WZptERGTIr05Qvyj3Vp&index=9',
  //   base: false,
  //   cifra: `
  //       D#º   Em9         C    G        D7
  //       A________mém, a--a--mém amém, amém
  //       D#º    Em9        C    G        D
  //       A________mém, a--a--mém amém, amém
  //   `,
  // },
  {
    titulo: 'Verbum Panis',
    id: sumId(),
    categoria: 'Comunhao',
    link: 'https://youtu.be/9-nL7F8RntA',
    base: false,
    cifra: `

    [Intro] C#m  B/C#  C#m  B/C#
            C#m  B/C#  A7M

    [Primeira Parte]
    C#m7        B/C#
    Desde o princípio
          C#m7       B/C#      C#m7        B/C#
    Antes mesmo que a terra começasse a existir
    C#m7     B/C#          C#m7  B/C#  C#m7  B/C#
    O verbo estava junto a Deus
    C#m7     B/C#
    Veio no mundo
          C#m7      B/C#           C#m7         B/C#
    E pra não abandonar-nos nesta viagem nos deixou
    C#m7       B/C#      C#m7  B/C#
    Todo a si mesmo como pão

    C#m7   B/C#         C#m7
    Verbum caro factum est
    C#m7    B/C#        C#m7
    Verbum panis factum est
    C#m7   B/C#        C#m7
    Verbum caro factum est
    C#m7   B/C#         A9   B9
    Verbum panis factum est

    [Refrão]
    E      B/D#         C#m7          B9
    E aqui partes o teu pão em meio a nós
          A9         E/G#  F#m7           B9
    Todo aquele que comer não terá mais fome
    E      B/D#      C#m7             B9
    E aqui vive tua igreja em torno a ti
        A9      E/G#  F#m7      B9
    Onde se encontrará a morada eterna

    C#m7
    Verbum caro factum est
    B9
    Verbum panis factum est
    A9
    Verbum caro factum est
    F#m7      G#m7
    Verbum panis

    [Primeira parte]

    C#m7
    Verbum caro factum est
    B9
    Verbum panis factum est
    A9
    Verbum caro factum est
    F#m7               A9   B9
    Verbum panis factum est

    [Refrão (2x)]

    C#m7
    Verbum caro factum est
    B9
    Verbum panis factum est
    A9
    Verbum caro factum est
    F#m7               A9   B9
    Verbum panis factum est
    `,
  },
  {
    titulo: 'Diante do Rei',
    id: sumId(),
    categoria: 'Gracas',
    link: 'https://www.cifraclub.com.br/vida-reluz/joia-rara/',
    base: true,
    cifra: `
    [Intro] E4

    [Primeira Parte]

    A2         E/G#
    Vem Senhor Jesus!
          F#m7                    C#m7
    O coração já bate forte ao te ver
            D7M           Bm7      C#m7
    A tua graça hoje eu quero receber
          Bm7                  E4       E
    Sem a benção do Senhor não sei viver

    A2           E/G#
    Vem Senhor Jesus!
            F#m7                         C#m7
    Olhar o povo ao teu redor me faz lembrar
            D7M        Bm7            C#m7
    A multidão lá no caminho a te esperar
          Bm7
    Vem ó Santo de Israel
        E4       E         E7
    Passar também neste lugar

    [Refrão]

        D7M            E/D
    É o rei! A nossa frente está!
        C#m7          F#m7
    É feliz quem o adorar
      D7M            E/D
    É Jesus, o nosso mestre e rei!
          C#m7             F#m7
    Bem aqui, tão perto se deixa encontrar
            D7M
    Diante do rei dos reis
    A/C#  Bm7       E4        E7
    Todo joelho se dobrará

    (volta  ao refrão)
    `,
  },
  // {
  //   titulo: 'Diante do Rei',
  //   id: sumId(),
  //   categoria: 'Gracas',
  //   link: 'https://www.cifraclub.com.br/vida-reluz/joia-rara/',
  //   base: true,
  //   cifra: `
  //   [Intro] E4

  //   [Primeira Parte]

  //   A2         E/G#
  //   Vem Senhor Jesus!
  //         F#m7                    C#m7
  //   O coração já bate forte ao te ver
  //           D7M           Bm7      C#m7
  //   A tua graça hoje eu quero receber
  //         Bm7                  A2/E
  //   Sem a benção do Senhor não sei viver

  //   A2           E/G#
  //   Vem Senhor Jesus!
  //           F#m7                         C#m7
  //   Olhar o povo ao teu redor me faz lembrar
  //           D7M        Bm7            C#m7
  //   A multidão lá no caminho a te esperar
  //         Bm7
  //   Vem ó Santo de Israel
  //       E7(4)
  //   Passar também neste lugar

  //   [Refrão]

  //       D7M            E
  //   É o rei! A nossa frente está!
  //       C#m7          F#m7
  //   É feliz quem o adorar
  //     D7M            E
  //   É Jesus, o nosso mestre e rei!
  //         C#m7             F#m7
  //   Bem aqui, tão perto se deixa encontrar
  //           D7M
  //   Diante do rei dos reis
  //   A/C#  Bm7      E4   A2
  //   Todo joelho se dobrará

  //   (volta  ao refrão)
  //   `,
  // },
  {
    titulo: 'O Senhor É Rei',
    id: sumId(),
    categoria: 'Final',
    link: 'https://www.cifraclub.com.br/anjos-de-resgate-musicas/seja-luz/',
    base: true,
    cifra: `
    Tom: G

    [Intro] Em  D  C  A  C
        Em  D  Em  D
        C  B7(4/9) B7(4)

    [Primeira Parte]

    Em                D/F#
      O Senhor é Rei
    G                          D  D#°
      O Senhor é meu pastor e Rei
    Em               D/F#
      O Senhor é Rei
    G                          D  B7(4/9) B7(4)
      O Senhor é meu pastor e Rei

    Em                      D/F#
      O Senhor está no céu
    G                      D4  D
      O Senhor está no mar
            C     B7   Em  C  A  D4  D
    Na extensão do infinito
    Em                      D/F#
      O Senhor está no céu
    G                      D4  D
      O Senhor está no mar
            C     B7   Em  C  A  D
    Na extensão do infinito

    [Refrão]

    G             Em  D          Bm7
      Está no céu,      está no mar
    Em          C          D
      Na extensão do infinito
    G             Em  D          Bm7
      Está no céu,      está no mar
    Em          C          D
      Na extensão do infinito

    ( Em  D  Em  D )
    ( C  Am7  B7(4/9) B7(4) )

    [Primeira Parte]

    Em                D/F#
      O Senhor é Rei
    G                          D  D#°
      O Senhor é meu pastor e Rei
    Em               D/F#
      O Senhor é Rei
    G                          D  B7(4/9) B7(4)
      O Senhor é meu pastor e Rei

    Em                      D/F#
      O Senhor está no céu
    G                      D4  D
      O Senhor está no mar
            C     B7   Em  C  A  D4  D
    Na extensão do infinito
    Em                      D/F#
      O Senhor está no céu
    G                      D4  D
      O Senhor está no mar
            C     B7   Em  C  A  D
    Na extensão do infinito

    [Refrão]

    G             Em  D          Bm7
      Está no céu,      está no mar
    Em          C          D
      Na extensão do infinito
    G             Em  D          Bm7
      Está no céu,      está no mar
    Em          C          D
      Na extensão do infinito

    G             Em  D          Bm7
      Está no céu,      está no mar
    Em          C          D
      Na extensão do infinito
    G             Em  D          Bm7
      Está no céu,      está no mar
    Em          C          D   Em
      Na extensão do infinito
    `,
  },
];

export default repertorioMissaFev;
