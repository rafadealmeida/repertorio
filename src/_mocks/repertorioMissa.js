let id = 1000;

// eslint-disable-next-line no-plusplus
const sumId = () => id++;

const repertorioMissa = [
  {
    titulo: 'Vigia esperando aurora',
    id: sumId(),
    categoria: 'Entrada',
    cifra: `
    Tom: A  || Capotraste : 2ºCasa

    C Bm E Am D G7M D
    G7M       D        G7M        G7M       D            G7M  G7M B7M C7M
    Vigia esperando a aurora, Qual noiva esperando o amor.
     C7M       Bb7M      E               Am      D        G7M    G7M Bb7M C7M
    É assim que o servo espera. A vinda do seu Senhor. (2x)

    (G7M F#M D)
              D                            C G7M
    1.Ao longe, um galo vai cantar seu canto,
    E   E7                Am
    O sol no céu vai estender seu mando,
    C          D              G
    Mas na muralha eu estarei desperto,
     Asus  Asus/Eb                D     C Bm D
    Que já vem perto o dia do Senhor.

         D                     G
    2.A minha voz vai acordar meu povo,
       E   E7                     Am
    Louvando a Deus, que faz o mundo novo.
       C             D7       G
    Não vou ligar, se a madrugada é fria,
       A7                         D    C Bm D­
    Que um novo dia logo vai chegar.

         D                     G
    3.Se é noite escura, acendo a minha tocha,
       E  E7                     Am
    Dentro do peito, o sol já desabrocha.
       C             D7       G
    Filho da luz, não vou ficar: vigio.
       A7                         D    C Bm D
    Ao mundo frio vou levar o amor!
    `,
  },
  {
    titulo: 'Hino da Imaculada Conceição',
    id: sumId(),
    categoria: 'Entrada',
    cifra: `
      C                              Em Ebm Dm
    1 – Salve! Salve! A Virgem Maria/ Imaculada Conceição/
           G7                      Dm        G7                         C
    Escolhida por Deus, Co-Redentora/ o teu sim trouxe ao mundo a salvação.
           Gm                              C         C/E      F
    És perfeita és cheia de graça/ Mãe de Deus és também nossa Mãe/
         Fm                   C     Am       Dm          G7          C
    Te exaltamos com louvor e oração./ Viva! A Virgem Imaculada Conceição!
    C                                               G
    Oh! Maria./ Oh! Maria!/ Aqui estamos com amor e devoção./
           F         Fm         C    Am
    Pra te aclamar padroeira Mãe amada./
              Dm       G7         C   Am
    Viva! A Virgem Imaculada Conceição/
                Dm      G7           C
    Viva! A Maria a Imaculada Conceição!

    2 – Virgem Santa os teus filhos hoje cantam/ Imaculada Conceição./
    Tu és Bondosa és Formosa, oh! Senhora./
    Nosso auxilio o modelo do cristão.
    És bendita entre todas as mulheres/
    És a porta do céu pra nós aberta/
    Te exaltamos com louvor e oração.
    /Viva! A Virgem Imaculada Conceição!
    3 –Hoje nós te consagramos Oh! Maria/ Imaculada Conceição!/
    Nossa vida, nossa casa, a família,/ nossa Igreja e o nosso coração./
    Medianeira tu és intercessora/ dos pequenos Mãe querida protetora/
    Te exaltamos com louvor em oração./
    Viva! A Virgem Imaculada Conceição
    `,
  },
  {
    titulo: 'Quero confessar a ti',
    id: sumId(),
    categoria: 'Penitencial',
    cifra: `
    [Intro] F#m7  D9  E  F#m7  D9  E4 E

      A7M            F#m       A7M         F#m
    Quero confessar a ti,     ilumina minh'alma
              Bm Bm7 E4 E        Asus2 A(7M9) E4 E9
    Eu reconheço:   sou pecador!
    Asus2 A(7M9) F#m11 F#sus4/7 Asus2 A(7M9) F#m11 F#sus4/7
    Diante de mim eu sei    está sempre o meu pecado
                Bm Bm7 E4     Asus2 A(7M9) E4 E9
    Foi contra vós     que eu pequei

    F#m    D9  E
    Kyrie eleison
    F#m       D9  C#m
    Christe eleison
    D   A/C#   Bm  E    Asus2 A(7M9)  E E9
    Kyrie    ele  ei  son
   Asus2 A(7M9)    F#m11 F#sus4/7 (2X)
    `,
  },
  {
    titulo: 'Salmo',
    id: sumId(),
    categoria: 'Salmo',
    cifra: `
    REFRAO
    E A B F#M A B
    E A B F#M A B E/G#
    ESTROFE
    A E/G# F#M B
    A E/G# F#M A B
    `,
  },
  {
    titulo: 'Aclamação',
    id: sumId(),
    categoria: 'Aclamacao',
    cifra: `
    Tom:D
    D   A   Bm  F#m  G    D  A  A7
    Aleluia,  aleluia,  aleluia
    D   A   Bm  F#m  G    D  A  A7
    Aleluia,  aleluia,  aleluia

    G             D/F#  E          F#M
    Quando estamos unidos, estás entre nós
    G         EM         (F)           A
    E nos falarás da Tua vida
    G             D/F#  E          F#M
    Este nosso mundo sentido terá
    G         EM         (F)           A
    Se Tua palavra renovar
        1.
    D   A   Bm  F#m  G    D  A  A7
    Aleluia,  aleluia,  aleluia
    D   A   Bm  F#m  G    D  A  A7
    Aleluia,  aleluia,  aleluia
    `,
  },
  {
    titulo: 'Uma noite de suor',
    id: sumId(),
    categoria: 'Oferta',
    cifra: `
    Tom:D
    [Intro] Bm  C#m  D D  A  F#m Bm  F#m  Bm

             Bm   C#m D
    Uma noite de suor sobre o barco em alto mar
            A           F#m
    O céu começa a clarear
          G           Bm        D
    A tua rede está vazia mas a voz que te chama
             A            F#m
    Te mostrará um outro mar
            G          D
    E sobre muitos corações
          Em        Bm  F#
    A tua rede lançará

    Bm         D          A       F#       G
    Doa a tua vida como Maria aos pés da cruz
    Bm  B9    Bm     D             A
    E   se....rás... Servo de cada homem
    D          G    D         Em       D   F#
    Servo por amor  sacerdote da humanidade

                         Bm  c#m   D
    Caminhavas no silêncio      esperando além da dor
            A               F#m
    Que a semente que tu lançavas
               G         Bm
    Num bom terreno germinasse
          D
    Mas o coração exulta
              A                 F#m
    Porque o campo já está dourado
             G         D
    O grão maduro pelo sol
         Em           Bm  F#
    No celeiro pode entrar

    Bm        D          A       F#m       G
    Doa a tua vida como Maria aos pés da cruz
    Bm  B9    Bm     D             A
    E   se....rás... servo de cada homem
    D           G    D         Em         D
    Servo por amor  sacerdote da humanidade
    `,
  },
  {
    titulo: 'Santo 2',
    id: sumId(),
    categoria: 'Santo',
    cifra: `
    [Intro] Em7  D9  G/B
    C  Am7  D/F#

    G      D/F#   C       D
    Santo  santo  san....anto
    G     D/F#        D/C   C  D/C  C
    Senhor Deus do universo

    G      D/F#   C       D
    Santo  santo  san....anto
    G     D/F#        D/C   C  D/C  C
    Senhor Deus do universo

    F                C              G
    Céus e terra proclamam a vossa glória
    F       C       D
    Hosana nas alturas
    F             C            G
    Bendito o que vem em nome do Senhor
    F       C      D
    Hosana nas alturas

    G      D/F#   C       D
    Santo  santo  san....anto
    G     D/F#        D/C   C  D/C  C
    Senhor Deus do universo

    G      D/F#   C       D
    Santo  santo  san....anto
    G     D/F#        D/C   C  D/C  C
    Senhor Deus do universo

    [Final] D/C  G

    `,
  },
  {
    titulo: 'Cordeiro',
    id: sumId(),
    categoria: 'Cordeiro',
    cifra: `
    [Intro]  C/E  D/F#  G

        G         Bm7               C9
    Cordeiro de Deus que tirais o pecado do mundo
    Am7       D                    G D/F#
    Tende piedade, piedade de nós
        G         Bm7               C9
    Cordeiro de Deus que tirais o pecado do mundo
    Am7       D                    G D/F#
    Tende piedade, piedade de nós
        G         Bm7               C9
    Cordeiro de Deus que tirais o pecado do mundo
    Am7     D D7  G
    Dai-nos a paz, a vossa paz
    `,
  },
  {
    titulo: 'Comunhão',
    id: sumId(),
    categoria: 'Comunhao',
    cifra: `
    Tom:A
    A               C#m     D     Bm          E
    É bom estarmos juntos Nesta mesa do Senhor
    A                  C#m  D       Bm             E
    E sentirmos Sua presença No calor de nosso irmão
    A                C#m           D9         BM     E
    Deus nos reune aqui Em um só espírito, Um só coração
    A             C#m              D9         BM       E
    Toda família vem Não falta ninguém Nesta comunhão

       A                                         B
    E vem cantando entre nós  Maria de Deus, Senhora da Paz
       D              BM      E                 A           E
    E vem orando por nós  A mãe de Jesus

       D              BM      E                 A           Bm  E    A
    E vem orando por nós  A mãe de Jesus

    A               C#m          D9  Bm      E
    Maria, nossa mãezinha Nos convida a união
    A                C#m         D9  Bm        E4  E
    sua presença nos une Faz-nos todos mais irmãos
    A               C#m            D9  Bm           E
    Nossa Senhora escuta O nosso silêncio, a nossa oração
    A               C#m               D9  Bm              E4  E
    E apresenta ao Filho Que se dá no vinho Que se dá no pão


    `,
  },
  {
    titulo: 'Ação de Graças',
    id: sumId(),
    categoria: 'Gracas',
    cifra: `
    Tom:G
    G                  Am  G  D/F#
      Não posso ca...minhar
        G      D/F#     Em
    Com minhas próprias forças
       Am       G D/F#   Esus4 E
    Conti......go é bem melhor
               Am7   G    D/F#
    Contigo o pou...co é muito
        G   D/F#   Em
    Sem ti o tudo é nada
       D  C       D                       Em
    Minha   vida é teu mandar
            D/F#                       C    D
    O teu caminho é o meu

                 G
    Não, eu não posso
        D/F#       Em7      D/F#
    Não vou dar um passo sequer
        C            G/B      Em7    D
    Se Deus não for comigo não irei jamais
      D              G
         Não, eu não posso
        D/F#       Em       D/F#
    Não vou dar um passo sequer
        C   G/B      Am                    D     G
    Se Deus não for comigo não irei   jamais
       C   G/B      Am                     D    C/G G
    Se Deus não for comigo não irei   jamais
    `,
  },
  {
    titulo: 'Final',
    id: sumId(),
    categoria: 'Final',
    cifra: `
    G  Am7  C  D

        G                          Bm7
    Uma vida nova em Cristo venha experimentar
        C                           D
    E de coração rendido, dia a dia se entregar
          G                         Bm7
    Com os olhos para o alto daqui menos depender
          C
    Para o mundo estamos mortos
          C#º                  D
    Nossa vida escondida está em Deus

    Nasci pro céu

    G              Am7            C              D
    Olhe, olhe mais longe, além do mundo, voe pro céu
    G               Am7               C
    Busque, busque o alto, pôr sobre a vida
        Cm6                 G      Am7
    Veja o trono, onde está Deus!
    C      D7           G
      (Alçando vôo se lançar)

          G                         Bm7
    Vem do céu essa alegria que me faz compreender
          C                 D
    Vejo além da agonia é certo eu vou vencer
        G                            Bm7
    Se levante para Cristo, não se arraste mais ao chão
          C                       C#º
    Meu irmão cabeça erguida, Ele trás a nova vida
      D
    O amor e o perdão, olhe pro céu

        Am7         D7          Bm7              E7/4   E7
    Se portanto ressuscitastes, buscai tudo que está no alto
    Am7          D7              C         Cm6       G   Bm7  C9  D
    Onde Cristo é tudo em todos, vem à vida nova se lançar
    C      D7           G
      (Alçando vôo se lançar)
    `,
  },
];

export default repertorioMissa;
