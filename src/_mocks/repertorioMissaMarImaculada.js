/* eslint-disable prettier/prettier */
let id = 700;

// eslint-disable-next-line no-plusplus
const sumId = () => id++;

const repertorioMissaMarImaculada = [
  {
    titulo: 'Alegrai-vos',
    id: sumId(),
    categoria: 'Entrada',
    cifra: `
    C            F            C  G
    Alegrai-vos sempre no Senhor
    C            F      G     C   G
    Alegrai-vos sempre no Senhor     (2x)
    C              F
    Não vos inquieteis com nada

    C        F              G
    em toda circunstância, orai ao Senhor
    Am      Em       Am              Em          F
    Apresentai a Deus as vossas preocupações com orações
    Dm              Bb      G
    Suplicas e louvor

    C           F         C                  F          G
    E a paz do nosso Deus, maior que toda inteligência, há de guardar
    Am     Em       Am       Em              F             Dm         Bb   G
    Vossos corações, vossos pensamentos, em Jesus Cristo, Nosso Senhor...
    `,
  },
  {
    titulo: 'Quero confessar a ti',
    id: sumId(),
    categoria: 'Penitencial',
    link: 'https://www.youtube.com/watch?v=sr9HxxIDToY',
    base: false,
    cifra: `
    [Intro] F#m7  D9  E  F#m7  D9  E4 E

      A7M            F#m       A7M         F#m
    Quero confessar a ti,     ilumina minh'alma
              Bm Bm7 E4 E        Asus2 A(7M9) E4 E9
    Eu reconheço:   sou pecador!
    Asus2 A(7M9) F#m11 F#sus4/7 Asus2 A(7M9) F#m11 F#sus4/7
    Diante de mim eu sei    está sempre o meu pecado
                Bm Bm7 E4     Asus2 A(7M9) E4 E9
    Foi contra vós     que eu pequei

    F#m    D9  E
    Kyrie eleison
    F#m       D9  C#m
    Christe eleison
    D   A/C#   Bm  E    Asus2 A(7M9)  E E9
    Kyrie    ele  ei  son

   Asus2 A(7M9)    F#m11 F#sus4/7 (2X)
    `,
  },
  {
    titulo: 'Salmo 18',
    id: sumId(),
    link: 'https://www.youtube.com/watch?v=DPFQWPRgd2E',
    base: false,
    categoria: 'Salmo',
    cifra: `
    INTRO: A9, E, C#m7, B, A9, E, B4, B

            E             B               C#m7
    QUE SE PRENDA A MINHA LÍNGUA AO CÉU DA BOCA,
          A9         B               E
    SE DE TI, JERUSALÉM, EU ME ESQUECER!

                C#m7        G#m7
    1. Junto aos rios da Babilônia
            A         G#m7
    nos sentávamos chorando,
          F#m7       B
    com saudades de Sião.
          C#m7        G#m7
    Nos salgueiros por ali
          A            B
    penduramos nossas harpas.

                C#m7           G#m7
    2. Pois foi lá que os opressores
          A           G#m7
    nos pediram nossos cânticos;
          F#m7       G#m7
    nossos guardas exigiam
        A          B
    alegria na tristeza:
            C#m7      G#m7
    “Cantai hoje para nós
          A         B
    algum canto de Sião!”

              C#m7        G#m7
    3. Como havemos de cantar
          A         G#m7
    os cantares do Senhor
        F#m7         B
    numa terra estrangeira?
          C#m7      G#m7
    Se de ti, Jerusalém,
          A             G#m7
    algum dia eu me esquecer,
            A             B
    que resseque a minha mão!

              C#m7         G#m7
    4. Que se cole a minha língua
          A              G#m7
    e se prenda ao céu da boca,
          F#m7           B
    se de ti não me lembrar!
          C#m7      G#m7
    Se não for Jerusalém
            A          B
    minha grande alegria!
    `,
  },
  {
    titulo: 'Aleluia',
    id: sumId(),
    categoria: 'Aclamacao',
    cifra: `
    Intro: Dm G Dm G C Dm
    Dm           G      Dm
    Louvor a vós, ó Cristo Rei,
    G      Dm     G      Dm            C  Dm
    Rei da eterna glória, Rei da eterna glória.


    Tanto Deus amou o mundo, que lhe deu seu Filho único;

    Todo aquele que crer nele há de ter a vida eterna.

    `,
  },
  {
    titulo: 'Venho Senhor Minha Vida Oferecer',
    id: sumId(),
    categoria: 'Oferta',
    base: false,
    cifra: `
    [Intro] Bm  E/D  C#m F#m
    Bm  C#m D  D/E
    A  D/E

    A                   E/G#      F#m F#m/E
    Venho senhor minha vida oferecer
    D           A/C#      Bm       E
    Como oferta de amor e sacrifício
    A            E/G#     F#m     F#m/E
    Quero a minha vida a ti entregar
    D            E          A      (G  A)
    Como oferta viva em teu altar

    D    E/D       C#m  F#m
    Pois pra te adoraaaaar
    Bm   E         F#m
    Foi que eu nasci
    C#m
    Cumpra em mim
    D         C#m7
    O teu querer
    Bm7          E             Em7  A7
    Faça o que está em teu coração
    D           E/D       C#m           F#m
    E que a cada dia eu queira mais e mais
    Bm         C#m7     D
    Estar  ao  teu  lado, senhor
    Bm         C#m7     D
    Estar  ao  teu  lado, senhor
    D                E       A
    Estar  ao  teu  lado, senhor
    `,
  },
  {
    titulo: 'Santo',
    id: sumId(),
    categoria: 'Santo',
    link: 'https://www.youtube.com/watch?v=vzykmWYLToo',
    base: false,
    cifra: `
    C               Am   Dm                 G7
    Santo, Santo, Santo, Senhor Deus do universo,
       C                 Am      F         G
    O Céu e a Terra proclamam a Vossa Glória.

    C              F C     F    G
    Hosana nas alturas, hosa-a-ana
    C              F C     F    G
    Hosana nas alturas, hosa-a-ana

    C               Am   Dm          G7
    Bendito o que vem em nome do Senhor
    C               Am   Dm          G
    Bendito o que vem em nome do Senhor

    C              F C     F    G
    Hosana nas alturas, hosa-a-ana
    C              F C     F    G
    Hosana nas alturas, hosa-a-ana

    `,
  },
  {
    titulo: 'Amém',
    id: sumId(),
    categoria: 'Novas',
    link: 'https://www.youtube.com/watch?v=ZpafvInYt1w&list=PL9F8jR3RBb9I30WZptERGTIr05Qvyj3Vp&index=9',
    base: false,
    cifra: `
    D#º   Em9         C    G        D7
    A________mém, a--a--mém amém, amém
    D#º    Em9        C    G        D
    A________mém, a--a--mém amém, amém
    `,
  },
  {
    titulo: 'Cordeiro',
    id: sumId(),
    categoria: 'Cordeiro',
    link: 'https://www.youtube.com/watch?v=9IUtklt8tNw&t=20s',
    base: false,
    cifra: `
    [INTRO] : G Bm C D4 D

        G                   D/F#             C     D4       D
    Cordeiro de Deus que tirais o pecado do mundo,tende piedade
        G                   D/F#             C     D4       D#º
    Cordeiro de Deus que tirais o pecado do mundo tende piedade
        Em                  Bm               Am   D4  D7(9-)
    Cordeiro de Deus que tirais o pecado do mundo

              G  D/F#                C/E
    Dai-nos a pa_______z,  dai-nos a paz
    D         Em     Bm           C
    Dai-nos a vossa paz, dai-nos a paz

    D         G  D/F#                C/E
    Dai-nos a pa_______z,  dai-nos a paz
    D         Em     Bm           C  D  G
    Dai-nos a vossa paz, dai-nos a paz
    `,
  },
  {
    titulo: 'Verbum Panis',
    id: sumId(),
    categoria: 'Comunhao',
    link: 'https://youtu.be/9-nL7F8RntA',
    base: false,
    cifra: `

    [Intro] C#m  B/C#  C#m  B/C#
            C#m  B/C#  A7M

    [Primeira Parte]
    C#m7        B/C#
    Desde o princípio
          C#m7       B/C#      C#m7        B/C#
    Antes mesmo que a terra começasse a existir
    C#m7     B/C#          C#m7  B/C#  C#m7  B/C#
    O verbo estava junto a Deus
    C#m7     B/C#
    Veio no mundo
          C#m7      B/C#           C#m7         B/C#
    E pra não abandonar-nos nesta viagem nos deixou
    C#m7       B/C#      C#m7  B/C#
    Todo a si mesmo como pão

    C#m7   B/C#         C#m7
    Verbum caro factum est
    C#m7    B/C#        C#m7
    Verbum panis factum est
    C#m7   B/C#        C#m7
    Verbum caro factum est
    C#m7   B/C#         A9   B9
    Verbum panis factum est

    [Refrão]
    E      B/D#         C#m7          B9
    E aqui partes o teu pão em meio a nós
          A9         E/G#  F#m7           B9
    Todo aquele que comer não terá mais fome
    E      B/D#      C#m7             B9
    E aqui vive tua igreja em torno a ti
        A9      E/G#  F#m7      B9
    Onde se encontrará a morada eterna

    C#m7
    Verbum caro factum est
    B9
    Verbum panis factum est
    A9
    Verbum caro factum est
    F#m7      G#m7
    Verbum panis

    [C#m7        B/C#
    Desde o princípio
              C#m7    B/C#       C#m7     B/C#
    Quando o universo foi criado da escuridão
    C#m7     B/C#           C#m7  B/C#  C#m7  B/C#
    O verbo estava junto a Deus
    C#m7     B/C#
    Veio no mundo
         C#m7     B/C#           C#m7         B/C#
    Rico em misericórdia deus mandou o filho seu
    C#m7      B/C#       C#m7  B/C#
    Todo a si mesmo como pão

    C#m7
    Verbum caro factum est
    B9
    Verbum panis factum est
    A9
    Verbum caro factum est
    F#m7               A9   B9
    Verbum panis factum est

    [Refrão (2x)]

    C#m7
    Verbum caro factum est
    B9
    Verbum panis factum est
    A9
    Verbum caro factum est
    F#m7               A9   B9
    Verbum panis factum est
    `,
  },
  {
    titulo: 'Yeshua',
    id: sumId(),
    categoria: 'Gracas',
    link: 'https://www.youtube.com/watch?v=lHdr7akL7xg',
    cifra: `
    [Intro] G#m  E  B  F# (2X)

    G#m
    Te chamam de Deus e de senhor
    E
    Te chamam de rei, de salvador
    B                                 F#
    E eu me atrevo a Te chamar de meu Amor
    G#m
    Te chamam de Deus e de senhor
    E
    Te chamam de rei, de salvador
    B                                 F#
    E eu me atrevo a Te chamar de meu Amor

    G#m     E
    Yeshua, yeshua
    B      F#
    Tu És tão lindo
              G#m
    Que eu nem sei me expressar
    E        B       F#
    Yeshua,tu És tão lindo

    G#m
    Te chamam de Deus e de senhor
    E
    Te chamam de rei, de salvador
    B                                 F#
    E eu me atrevo a Te chamar de meu Amor
    G#m
    Te chamam de Deus e de senhor
    E
    Te chamam de rei, de salvador
    B                                 F#
    E eu me atrevo a Te chamar de meu Amor

    [Refrão] (2X)
    G#m     E
    Yeshua, yeshua
    B      F#
    Tu És tão lindo
              G#m
    Que eu nem sei me expressar
    E        B       F#
    Yeshua,tu És tão lindo

    ( G#m  E  B  F# )
    `,
  },
  {
    titulo: 'A Alegria Está No Coração',
    id: sumId(),
    categoria: 'Final',
    link: 'https://youtu.be/FnnmK-hOXEM',
    base: false,
    cifra: `
    [Intro] A  G  D

    A                               D                  A  G D
    A alegria está no coração, de quem já conhece a Jesus
    A                                    B              E
    A verdadeira paz só tem aquele, que já conhece a Jesus
    A                   A7          D               Dm
    O sentimento mais precioso que vem do nosso Senhor
      A           E                        A  G D
    É o amor  que só tem, quem já conhece a Jesus

    A                               D                  A  G D
    A alegria está no coração, de quem já conhece a Jesus
    A                                    B              E
    A verdadeira paz só tem aquele, que já conhece a Jesus
    A                   A7          D               Dm
    O sentimento mais precioso que vem do nosso Senhor
      A           E                        A    E
    É o amor  que só tem, quem já conhece a Jesus
    `,
  },
];

export default repertorioMissaMarImaculada;
