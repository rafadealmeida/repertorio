let id = 1;

// eslint-disable-next-line no-plusplus
const sumId = () => id++;

const cifra = [
  {
    titulo: 'Nascer de novo',
    id: sumId(),
    categoria: 'Espirito',
    cifra: `
    Am                            G
    Em verdade, em verdade te digo
      F                    G   Am         G    F             G
    Quem não nascer de novo, não poderá ver, o reino de Deus (bis)

             Am          G                 F
    Por isso vem, vem, vem, vem, vem,     vem
    G
       Vem Espírito Santo (bis)

      Am                                          G
    Em verdade, em verdade te digo
      F             G      Am       G         F                        G
    Quem não renascer, da água e do Espírito Não entrará no reino de Deus

             Am           G                F
    Por isso vem, vem, vem, vem, vem,     vem
    G
       Vem Espírito Santo (bis)

      Am                   G
    O vento sopra onde quer
     G              E       Am                G   F              G
    Você ouve o ruído, mas não sabe de onde vem Nem pra onde vai
    Am               G                F           G
    Assim acontece com aquele que nasceu do espírito (bis)
            Am            G                F
    Por isso vem, vem, vem, vem, vem,     vem
    E
       Vem Espírito Santo (bis)
    `,
  },
  {
    titulo: 'Espírito da Verdade',
    id: sumId(),
    categoria: 'Espirito',
    cifra: `
    [Intro] C  C9  F  C  C9  F

    C              C9
    Espírito da verdade
    C              C9
    Espírito da verdade
        Am
    Ouça a minha voz

        F                     G
    Já não quero o que não te agrada
        F                G
    Renuncio o que nos afasta

    C              Am
    Sopra dos quatro cantos
            F
    Espírito Santo
                C              G
    E enche meu coração, meu coração

    `,
  },
  {
    titulo: 'NOSSO GENERAL /PRECISO DE VOCÊ',
    id: sumId(),
    categoria: 'Decolagem',
    cifra: `
    GM
    Pelo senhor, marchamos sim, seu exército poderoso é,
          Eb         F              GM      GM
    Sua glória será vista em toda terra. Vamos cantar o canto da vitória:
                                        Eb               F             GM
    Glória a deus, vencemos a batalha, toda arma contra nós perecerá...
    Eb                    Bb
    O nosso general é cristo,
    Eb                    Bb
     Seguimos os seus passos,
    Eb              F     GM
    nenhum inimigo nos resistirá!... (2X)

    F              Dm Bb                C      F         Dm            Bb       C
    Eu preciso de você, você precisa de mim, Nós precisamos de cristo até o fim.
         F                         Dm            Bb                C
    Sem cessar, sem parar, sem vacilar, sem tremer, sem chorar...

    F          Dm        Bb             C
    Se amar é viver eu vivo porque amo você.
    `,
  },
  {
    titulo: 'Espirito Santo Repousa',
    id: sumId(),
    categoria: 'Espirito',
    cifra: `
    [Intro] A A5 A6 A7 BM E7
    A   A5      A6        A7      BM                   E7
    Espírito Santo repousa,      Espírito Santo, repousa
     A  A5      A6        A7      BM             E7
    Espírito Santo repousa,      Espírito Santo, repousa

     D               A/C#       BM                  Esus
    Sobre nós, sobre todos nós Espírito Santo, repousa
    D                 A/C#      BM                E4
    Sobre nós, sobre todos nós Espírito Santo repousa sobre nós

    A  A5      A6        A7      BM                    E7
    Espírito Santo repousa,      Espírito Santo, repousa
    A  A5      A6        A7      BM               E7
    Espírito Santo repousa,      Espírito Santo, repousa

     D                 A/C#          BM                Esus
    Traz unção, traz unção a nós Espírito Santo repousa
     D                 A/C#          BM          E4         D         A/C#
    Vem curar nossos corações Espírito Santo repousa sobre nós
              BM       Esus            D    A/C#
    Espírito Santo repousa sobre nós
              BM       E4              D    A/C#
    Espírito Santo repousa sobre nós
              BM       Esus                A
    Espírito Santo repousa sobre nós
    `,
  },
  {
    titulo: 'QUERO ESTA EM TUA MÃOS OH PAI',
    id: sumId(),
    categoria: 'Louvor',
    cifra: `
    E
    Eu quero estar em Tuas mãos, Oh Pai !
         A             B/A    E     A B/A
    Eu quero depender de Ti (bis)
         A                 E
    Eu vou louvar  no Espírito
         A            B/A     E    A  B/A
    Eu quero depender de Ti (Bis) (Refrão)
    Eu vou cantar... Eu vou dançar...Eu vou pular...
    `,
  },
  {
    titulo: 'LOUVA A DEUS',
    id: sumId(),
    categoria: 'Louvor',
    cifra: `
    [Intro] C  G  Am  Em  F  G  C   F  G
        C        G                 Am  Em            F         G               C   F G
    Louva a Deus,       louva a Deus,      e ao Senhor Jesus de quem as bênçãos vêm
        C        G                 Am  Em            F         G              C
    Louva a Deus,       louva a Deus,      e ao Senhor Jesus de quem as bênçãos vêm
        Em        F     Bb7/9           Am          G             C
    Se o coração abrir,         águas vivas do interior irão fluir
        Em        F     Bb7/9           Am          G            C
    Se o coração abrir,         águas vivas do interior irão fluir
    ( F G )
    ( C  G/B  Am  Em  F  G  C   F  G)

              C G                    Am  Em           F            G           C   F G
    E ao seu nome,        louva então,       nunca mais serás o mesmo meu irmão
            C G                     Am  Em           F             G          C
    E ao seu nome,        louva então,       nunca mais serás o mesmo meu irmão

    `,
  },
  {
    titulo: 'CONSAGRAÇÃO A NOSSA SENHORA',
    id: sumId(),
    categoria: 'Maria',
    cifra: `
    A                  C#M               CM BM      E            A
    Ó minha senhora e também minha mãe, eu me ofereço inteiramente todo
        F#M  BM     E         A       F#M    BM      E         A   E7
    A vós e em prova da minha devoção, eu hoje vos dou meu coração.
    A                                  C#M
    Consagro a vós meus olhos, meus ouvidos, minha boca,
    BM         E               A     F#M          BM   E
    Tudo o que sou desejo que a vós, perten__ça, incomparável mãe
    A           F#M         BM               E             A     F#M
    Guardai-me defendei-me como coisa e propriedade vossa  amém, como
    BM            E              D DM A
    Coisa e propriedade vossa amém.

    `,
  },
  {
    titulo: 'QUEM É ESTA QUE AVANÇA COMO A AURORA',
    id: sumId(),
    categoria: 'Maria',
    cifra: `
    C#M EB E7M B

    Quem é esta que avança como Aurora

    Temível como exército em ordem de batalha

    Brilhante como o sol e como a lua

    Mostrando os caminhos aos filhos seus

    Ah, ah, ah, minha alma glorifica ao Senhor

    Meu Espírito exulta

    Em Deus, meu Salvador
    `,
  },
  {
    titulo: 'APERTE A MINHA MÃO',
    id: sumId(),
    categoria: 'Paz',
    cifra: `
    G                                        Em
    Aperte a minha mão irmão quero de dá a paz a paz
    C           D/F#         G                       Em                 C      D/F#
    Do meu Senhor, me de um abraço forte bem mais forte do que forte, um gesto de amor.
    C             D/F#       Bm7        Em7        C           D/F#     G G7
    A paz do Senhor quero te dá meu irmão com toda alegria que existe no meu coração.
    `,
  },
  {
    titulo: 'ISAÍAS 40',
    id: sumId(),
    categoria: 'Decolagem',
    cifra: `
    G          F                G     F
    Mas os que esperam no senhor
    G          F             E4 E7
    Renovarão as suas forças,
    AM EM           F          G      G F# F
    Subirão com asas como águias,
              G F# F                 G   G  F# F
    Correrão       e não se  cansarão,
                 G       F                E E4
    Caminharão e não se fatigarão. (bis)
    `,
  },
  {
    titulo: 'AMIGO',
    id: sumId(),
    categoria: 'Decolagem',
    cifra: `
    E                    F#M          B7                          E   E7
    Amigo alegria em ter você aqui / me faz sentir vontade de cantar e lhe dizer
    A               B7          G#M       C#M       F#M       B7          E E7
    Que cristo quer lhe oferecer a alegria de viver aceito o seu amigo ele será
    A              B7          G#M          C#M           F#M       B7            E
    Queremos que você volte outra vez e novamente lhe diremos que foi bom conhecer você
    C#M         F#M     B7         E        C#M          F#M      B7       E
    foi Bom / foi bom conhecer você / foi bom / foi bom conhecer você. (2x)
    `,
  },
  {
    titulo: 'FELIZ COM JESUS',
    id: sumId(),
    categoria: 'Decolagem',
    cifra: `
    D A E   A   E      D       A      E DA E D A E D A E
    1.eu hoje estou muito feliz   com jesus
    DA  A   E      D       A      E DA E D A E D A E
    Eu hoje estou muito feliz com jesus
    F#M     B          E            DA   E
    Aleluia, aleluia, aleluia, amém
    2.você só será feliz com jesus (2x)
    3.você só terá a paz...
    4.você terá amor...
    5.você só se salvará...
    `,
  },
  {
    titulo: 'PODEROSO DEUS',
    id: sumId(),
    categoria: 'Espirito',
    cifra: `
    C                          G/B
    Sopra Espírito de Deus
    Am                            F
    Sobre o clamor da Igreja
    C                   G/B
    Libera Tua unção de milagres
        Am  F
    Sobre nós

            C          G/B           Am                           F
    Queremos ver sinais da manifestação do Teu poder
                    C      G/B    Am                             F
    Pois quem crer verá a gloria do Senhor acontecer

            C       G/B
    Poderoso Deus, e Rei
                Am                          F
    Libera tua unção de cura nesse lugar
            C              G/B
    Poderoso Deus, e Rei
                Am                          F
    Libera tua unção de cura nesse lugar

    (Repete)

    C             G/B            Am  F
    Incendeia Senhor, a Tua Igreja
    C             G/B            Am  F
    Incendeia Senhor, a Tua Igreja

    `,
  },
  {
    titulo: 'ESPÍRITO ENCHE MINHA VIDA',
    id: sumId(),
    categoria: 'Espirito',
    cifra: `
    D   BM     G                    A               G               A
    Espírito enche a minha vida, enche-me com teu
    F#M         BM                 G             A4   A      D
    Poder, pois a ti eu quero ser, espírito enche o meu
    A
    Ser (2x)
    G                A                    D  A  BM      G         A
    As minhas mãos eu quero levantar,  e em louvor ti
    BM    G            A                    D A/C# BM        EM
    Adorar,  meu coração eu quero derramar,  diante
    G/A    D
    Do teu altar.
    `,
  },
  {
    titulo: 'Move-te Em Mim',
    id: sumId(),
    categoria: 'Espirito',
    cifra: `
    D                G
    O Espírito de Deus está
           D   G
    Neste lugar
          D
    O Espírito de Deus
        G                A
    Se move neste lugar
           D             G
    Está aqui para consolar
           D             G
    Está aqui para Libertar
           Bm
    Está aqui para guiar
        A9
    O Espírito de Deus
           D  D4
    Está aqui      (2x)

                G  A
    Move-te em mi__im
               Bm
    Move-te em mim
                G             A
    Toca minha mente meu coração
                 D          Bm
    Enche minha vida do teu amor
                G          A
    Move-te em mim Deus Espírito
               D  D4
    Move-te em mim (2x)

    Estrofe
    Refrão
    Refrão Final calmo tar.

    `,
  },
  {
    titulo: 'JESUS FILHO DE DAVI',
    id: sumId(),
    categoria: 'Perdao',
    cifra: `
    [Intro] C  F  C  F

        C                        G
    Me contaram que o Senhor ia passar
            Dm                      G
    E que havia uma chance de eu poder enxergar
        C                   G
    Me disseram que tinhas poder
        Dm                       G
    Pra ressuscitar e fazer o paralítico andar

    Am                     Em    F
    Renasceu dentro de mim a fé
                        G
    Que há muito eu já não tinha
    Am                   Em    F
    Eu senti meu coração arder
                        G
    E eu gritei para o Senhor me responder:

    C                    G     Am
    Jesus, filho de Davi, me cura
        F           C
    És o Santo de Israel
                        G       Am
    Diante de Ti a tempestade se cala
            G             C
    Meu Deus honra a minha fé  (2x)
    `,
  },
  {
    titulo: 'RENOVA-ME',
    id: sumId(),
    categoria: 'Perdao',
    cifra: `
    D           G       A          D G      D/F#                Em A
    Renova-me Senhor Jesus.Já não quero ser igual
        D           G      A          D   G         D/F#               Em  A
    Renova-me Senhor Jesus. Põe em mim teu coração
     D                   A/C#     Bm      F#m    G   Em            G           A
    Porque tudo que  há dentro de mim. Precisa ser mudado Senhor
    D                     A/C#                       Bm    F#m   G          A              D
    Porque tudo que  há, dentro do meu  coração. Precisa mais de ti...

    `,
  },
  {
    titulo: 'CURA, SENHOR',
    id: sumId(),
    categoria: 'Perdao',
    cifra: `
    D             A                 Bm Bm/A
    1.Vamos Jesus passear na minha vida
    Em                  A                   Am D7
    Quero voltar aos lugares em que eu fiquei só
    G                        A             F#m                   Bm
    Quero voltar lá contigo, vendo que estavas comigo
    Em                       A                   D Am7  D7
    Quero sentir teu amor a me embalar.
    G                         A        F#m                  Bm
    Cura Senhor onde dói, cura Senhor bem aqui
    Em                         A                   Am D7
    Cura Senhor onde eu não posso ir
    G                        A         F#m                    Bm
    Cura Senhor onde dói, cura Senhor bem aqui
    Em                               A                   D
    Cura Senhor onde eu não posso ir

    2.Quando a lembrança me faz adormecer,/ sabes que a espada da dor entra em meu ser / Tu me carregas nos braços,/ leva-me com teu abraço / Sinto minh'alma chorar junto de ti .
    3.Tantas lembraças eu quero esquecer, / deixam vazio em minh'alma e em meu viver/
    Toma Senhor, meu espaço, / te entrego todo cansaço / Quero acordar com tua paz a me aquecer.
    `,
  },
  {
    titulo: 'VASO NOVO',
    id: sumId(),
    categoria: 'Perdao',
    cifra: `
    D9              Em7                       G
    Eu quero ser  Jesus amado como um barro
              A            D                                       D7                   G     Gm
    Nas mão  do oleiro, rompe-me  a vida.      Faze-me de novo
                       D   Bm           Em   A                G  D
    Eu quero ser,   eu quero ser           um vaso novo.
    `,
  },
  {
    titulo: 'REUNIDOS AQUI',
    id: sumId(),
    categoria: 'Louvor',
    cifra: `
        Intro. F F#  G
        G                                C
        Reunidos aqui, só pra louvar o Senhor
                    D         G
        Novamente aqui, em união
        F F#  G
        Algo bom vai acontecer
        F F# G                   C
        Algo bom Deus tem para nós !
                    D                      C   D      G
        Reunidos aqui, só pra louvar o Senhor !
    `,
  },
  {
    titulo: 'TE LOUVO EM VERDADE',
    id: sumId(),
    categoria: 'Louvor',
    cifra: `
    [Intro]  E  F#m  G#m  A  E  B9  E  B9

    E              F#m   G#m                  A9
   Mesmo na tempestade, mesmo que se agite o mar
    E            B9          E    B9
   Te louvo, te louvo em verdade

    E               F#m      G#m       A9
   Mesmo longe dos meus, mesmo na solidão
    E            B9          E   E7
   Te louvo, te louvo em verdade

           A9            B9    G#m           C#m
   Pois somente tenho a Ti, Tu és a minha herança
    A9           B9          E   E7
   Te louvo, te louvo em verdade

           A9            B9    G#m           C#m
   Pois somente tenho a Ti, Tu és a minha herança
    A9           B9
   Te louvo, te louvo em verdade

   E  F#m  G#m  A  E  B9  E  B9


   Mesmo que me faltem palavras
   Mesmo que eu não saiba louvar
   Te louvo, te louvo em verdade
   Mesmo que me faltem palavras
   Ainda que eu não saiba louvar
   Te louvo, te louvo em verdade
    `,
  },
  {
    titulo: 'SENHOR TU ÉS O MEU DEUS FORTE',
    id: sumId(),
    categoria: 'Louvor',
    cifra: `
    Em              G                    A                Em
    Senhor tu és o meu Deus forte.A minha rocha e  a minha luz
    Em                     G                A
    Por isso cada ver que eu olho pra ti eu sinto uma água viva
        Em                            Em     G       A
    Que sai de dentro de mim! ( Aleluia, Aleluia (2X) )
    `,
  },
  {
    titulo: 'REI SENHOR',
    id: sumId(),
    categoria: 'Louvor',
    cifra: `
    Intr:    (C Bb F ) (C Bb/C F/C)
    C           Gm/C       C         Gm/C
    Rei Senhor,         Rei Senhor
    C                            C/E Gm                Gm/BB      C                    C/E Gm                 Gm/Bb
    Vem louvar o Senhor, bendizer o seu nome, e ver que essa dor      do teu peito some
    F                   G    F                   G F                 G     F                            D
    Levanta o braço, abre o coração, morre pra ti mesmo e terás a paz então.
    C           Gm/C       C         Gm/C
    Rei Senhor,         Rei Senhor
    C                            C/E Gm        Gm/BB C                   C/E Gm                Gm/Bb
    Vem cantar ao Senhor, criador de tudo e receber alegria que não tem no mundo.
    F              G    F       G  F          G     F               D      C           Gm/C       C         Gm/C
    Bate palma, bate o pé dá um sorriso com muita fé. Rei Senhor,         Rei Senhor
    C                            C/E Gm           Gm/BB   C                            C/E Gm                Gm/Bb
    Vai confessar ao Senhor os teus pecados. Não , não tenhas medo, pois serás perdoado.
    F              G   F        G    F                      G F                     D         C           Gm/C       C         Gm/C
    Jesus te ama, meu irmão, entrega-te à Ele e terás a salvação. Rei Senhor,         Rei Senhor
    `,
  },
  {
    titulo: 'HOJE É TEMPO DE LOUVAR A DEUS',
    id: sumId(),
    categoria: 'Louvor',
    cifra: `
    D             Em         F#m  Em          D   Em            F#m  Em
    Hoje é tempo de louvar a Deus.  Em nós agora habita seu Espírito
    G       A      D        G       A      D              G           Em     A    A7
    E então é só cantar e a Cristo exaltar, e sua Glória encherá este lugar,
             D      Em F#m  Em
    Vem  louvar,   Vem   louvar (bis)
    D           Em            F#m  Em          D           Em           F#m  Em
    No meio dos louvores Deus habita, e é seu  prazer cumprir o que nos diz.
    Refr.: (Glória a Deus)
    `,
  },
  {
    titulo: 'ALEGRIA',
    id: sumId(),
    categoria: 'Louvor',
    cifra: `
    G                                C
    A alegria está  no coração de quem já
                    D  G
    conhece a Jesus, a verdadeira paz só tem
                    C              D D7
    aquele que já conhece a Jesus,
    G                                C
    O sentimento mais precioso que vem do
             Cm        G Em        Am
    nosso Senhor, é o amor que só tem quem
          D7       G   D   G
    já conhece a Jesus.
    G                           G7                           C Cm
    Posso pisar numa tropa e saltar as muralhas (Aleluia).(2X)
    `,
  },
  {
    titulo: 'QUERO LOUVAR-TE',
    id: sumId(),
    categoria: 'Louvor',
    cifra: `
    E
    Quero louvar-te, sempre mais e mais,
                C#m7
    quero louvar-te,         sempre mais e mais.
        A                 G# F#m7     D9                   B7
    Buscar o teu querer, tua graça conhecer, quero louvar-te.
        E
    As aves do céu cantam para ti,
              C#m7
    as feras do campo refletem teu poder.
      A9               G# F#m7                     B7
    Quero cantar, quero levantar as minhas mãos a ti.
                E
    Quero amar-te...
    Quero servir-te...
    Quero buscar-te...
    `,
  },
  {
    titulo: 'Meu tudo',
    id: sumId(),
    categoria: 'Espirito',
    cifra: `
    [Intro] C#m  A  F#  A  B

    E
    Sopra sobre mim Senhor
    C#m
        Uma brisa leve, que
    A
    Toque bem profundo
        F#          C#m  B
    Em meu ser, em meu ser

    E
    Vem espírito de Deus
    C#m
    Cura o meu coração
    A
    E faz me um homem novo
        F#
    Com o teu poder
        A7+    B      (A/C#)
    Com o teu poder (2x)

    B/D#    E         B/D#      C#m
    Te adorarei, renderei um louvor
    G#m7         A          E/G#
    Pois tu és Senhor o meu tudo
    F#m          B
    Me prostrarei a ti
            E         B/D#      C#m
    Te adorarei, renderei um louvor
    G#m7         A          E/G#
    Pois tu és Senhor o motivo
        F#m7    B        D7+   E  D7+  E
    E a minha razão de viver  (2x)
    `,
  },
  {
    titulo: 'Vem, vem louvar',
    id: sumId(),
    categoria: 'Louvor',
    cifra: `
    D           A7       G           A7          D Bm
    Vem! Vem louvar! Encher este lugar de glória!
    Em          A7      D  Bm     Em              A7
    Encher este lugar de glória, com a glória do Senhor
    D           A7       G           A7          D Bm
    Vem! Vem louvar! Encher este lugar de glória!
    Em          A7      D  Bm      Em             A7 D
    Encher este lugar de glória, com a glória do Senhor

            D    Am   D7      G           Gm
    Quando Deus envia Seu espírito, nos conduz à fé
        D    G             A7        D      Am   D7   G
    E nos faz cantar o Seu louvor. Seu amor em cada coração
        Gm                 D    G             A      G              A7
    Nos garante a paz e nos faz cantar o Seu louvor. (cante forte ao Senhor)

        D   Am    D7      G         Gm                D
    É tão bom estar neste lugar de alegria e paz que nos faz
    G             A7        D       Am    D7      G
    Cantar o Seu louvor! Eu também sou templo do Senhor
            Gm            D         G            A     G                 A7
    E o meu coração vai cantar pra sempre o Seu louvor. (cante a glória do senhor)
    `,
  },
  {
    titulo: 'Manda Teus Anjos',
    id: sumId(),
    categoria: 'Espirito',
    cifra: `
    [Intro] D  G/D  D  G/D

    D            G          D
   Manda teus anjos, sobre nós
          Bm         G             A
   E abençoa todos que esperam em vós
              D  G/D           D    Bm
   Manda teus anjos, pra nos ensinar
            Em      A     D  D4
   A te louvar, e glorificar

   D                   G         A      D
   Envia também teu espírito de paz e amor
          F#m         Bm   Em         A
   O meu coração tem sede,do meu criador
   D                    G               A    D
   Envia senhor os teus anjos, pra nos resgatar
             F#m        Bm        Em         A       D
   Pra nos proteger de todo mau, para nos guiar, senhor

   D            G          D
   Manda teus anjos, sobre nós
          Bm         G             A
   E abençoa todos que esperam em vós
              D  G/D           D    Bm
   Manda teus anjos, pra nos ensinar
            Em      A     D  D4
   A te louvar, e glorificar

     D                   G             A      D
   Quando acordo olho o céu e canto o meu louvor
       F#m         Bm     Em           A
   De todas as manhãs, tu és o meu senhor
         D             G              A    D
   Levantai-nos ó meu Deus e estende tuas mãos
      F#m         Bm         Em          A4      D
   Tu és o meu refúgio, nas minhas opressões senhor

       G        (A  G)     A       F#m       G  A
   Confio em vós,      revele tua face para nós
      G       (A  G)    A        F#m           G  F#m  Em  A
   Levanta-te,       e põe o teu escudo sobre nós

   Manda teus anjos

   [Solo] D  G  A  F#m  Bm  Em  A  D  B7|

    E            A          E
   Manda teus anjos, sobre nós
          C#m       A              B
   E abençoa todos que esperam em vós
              E  A              E B/D# C#m
   Manda teus anjos, pra nos ensinar
            F#m    B     E  C#m
   A te louvar, e glorificar
            F#m    B     E   C#m
   A te louvar, e glorificar
            F#m   B  B4
   A te louvar, e te    amar

   [Final] E  A/E  E  A/E  E
    `,
  },
  {
    titulo: 'Incendeia Minha Alma',
    id: sumId(),
    categoria: 'Louvor',
    cifra: `
    [Intro] C  Am  Dm7  G4
    C  Am  Dm7  G  G4

    [Primeira Parte]

    C           G/B         Am
    Espírito Santo, vinde
            G
    Falar em mim
    C           G/B         Am
    Espírito Santo, vinde
            G
    Falar em mim (2X)

    [Pré-Refrão]

        C/E  F               Am
    Vinde curar, vinde libertar
                Dm      C/E  G
    Nossos corações de toda  opressão
        C/E        F              Am
    Vinde transformar, vem incendiar
                D           G
    Traz fogo do céu nesse lugar

    [Refrão]

        C              G/B
    Incendeia minha alma
        Am             Am/G
    Incendeia minha alma
        F         C/E       G4  G
    Incendeia minha alma, Senhor    (2x)

    [Primeira Parte]

    [Pré-Refrão]

    [Refrão]

    `,
  },
  {
    titulo: 'Sede Santos',
    id: sumId(),
    categoria: 'Espirito',
    cifra: `
    [Intro]  G  D  C  Am  D

    G                 D/G
    Deus consagrou um povo escolhido
    C            Am7            C/D
    O amou profundamente, desde toda a eternidade:
    G
    Para ser sal e luz
       D/F#               Am7 Bm C        C/D
    Gerar Cristo Jesus no se..io   da humanidade (2x desde o início)
         Dm7          G7      E4    E
    Hoje somos esse povo tão ama...do
              Am7      Am/G        Cm
    Deus nos chama a viver o seu amor
            G             D/F#           Em7
    Nos convida a todo instante, sem cessar
      Am7     C/D           G     C/D
    E pede a nós:     sede santos

    [Refrão]

          G           D/F#
    Sede santos, esta é minha vontade
          C  Am    C/D                                                 C Am7 E4 E
    Sede santos, é o que mais quero... (2x desde o início do refrão) "Sede san..tos"
              A        E/G#
    Vou ser santo, esta é minha resposta
            D Bm E    E7sus4
    Vou ser santo, eu digo sim, Senhor ...

    `,
  },
];

export default cifra;
